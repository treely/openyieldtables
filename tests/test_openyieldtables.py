...
@pytest.mark.asyncio
async def test_get_yield_tables_meta_from_df():
    await df_manager.load_yield_tables_data()
    await df_manager.load_yield_tables_meta()
    yield_tables_meta_df = await df_manager.get_yield_tables_meta()
    yield_tables_meta = get_yield_tables_meta_from_df(yield_tables_meta_df)
...
@pytest.mark.asyncio
async def test_get_yield_table_from_df():
    await df_manager.load_yield_tables_data()
    await df_manager.load_yield_tables_meta()
    yield_tables_data_df = await df_manager.get_yield_tables_data()
    yield_tables_meta_df = await df_manager.get_yield_tables_meta()

    yield_table_data = get_yield_table_from_df(
        1, yield_tables_meta_df, yield_tables_data_df
    )
    assert yield_table_data.data.yield_classes[5].rows[0].model_dump() == {
        "age": 20,
        "dominant_height": 4.8,
    ...
    yield_table_data = get_yield_table_from_df(
        93, yield_tables_meta_df, yield_tables_data_df
    )
    assert yield_table_data.data.yield_classes[0].rows[0].model_dump() == {
        "age": 100,
        "dominant_height": 8.1,
    ...
    yield_table_data = get_yield_table_from_df(
        144, yield_tables_meta_df, yield_tables_data_df
    )
    assert yield_table_data.data.yield_classes[0].rows[0].model_dump() == {
        "age": 10,
        "dominant_height": None,
    ...
    # yield_class is a float
    yield_table_data = get_yield_table_from_df(
        102, yield_tables_meta_df, yield_tables_data_df
    )
    assert yield_table_data.data.yield_classes[0].yield_class == 0.4
    assert yield_table_data.data.yield_classes[8].yield_class == 3
    ...
    with pytest.raises(ValueError):
        get_yield_table_from_df(
            999, yield_tables_meta_df, yield_tables_data_df
        )
